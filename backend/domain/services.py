...
class LabelPolicy:
    ...
    per_source_thresholds: Optional[Mapping[str, float]] = None
    per_source_reason_templates: Optional[Mapping[str, str]] = None

    def get_threshold(self, *, source: str, default_threshold: float) -> float:
        """Return the applicable confidence threshold for the evaluation source."""

        if self.per_source_thresholds and source in self.per_source_thresholds:
            return self.per_source_thresholds[source]
    ...
    def get_reason(
        ...
        *,
        ...
        template = (
            (self.per_source_reason_templates or {}).get(source)
...
class ThresholdBusinessRulesEngine:
    ...
    def _evaluate_label(
        ...
        *,
        ...
        threshold = (
            policy.get_threshold(source=source, default_threshold=self.confidence_threshold)
            if policy
